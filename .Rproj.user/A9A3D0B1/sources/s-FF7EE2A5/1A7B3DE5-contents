...
cadre_harmonise_caf_ipc_filtered <- cadre_harmonise_caf_ipc %>% 
  filter(exercise_year == 2020 & exercise_label == "Jan-May" & chtype == "current") 
...
wca <- c("Benin","Burkina Faso","Cameroon","Central African Republic","Chad","Côte d'Ivoire","Gambia","Guinea","Liberia","Mali","Niger","Nigeria","Senegal","Sierra Leone","Togo")
...
wca_CH <- bind_rows(wca_adm1_filtered, wca_adm2_filtered, ner_adm2_5, mli_adm3) 
...
wca_CH_nga <- wca_CH %>% filter(adm0_pcod2 == "NG") %>% dplyr::select(-adm0_pcod2,-adm2_5_pcod2,-adm3_pcod2)
...
wca_final_CH_FINAL <- bind_rows(wca_final_CH, wca_final_CH_nga, wca_final_CH_ner) %>% dplyr::select(adm0_name, adm0_pcod3, adm0_pcod2, 
...
#create map this variable  and identify camp in CAR
...
map_phase_class <- ggplot()  +geom_sf(data = wca_final_CH_FINAL, mapping = aes(fill = as.factor(phase_class), color = NULL)) +theme_void() +coord_sf(datum=NA) +labs(fill="phasing")  +scale_fill_manual(values = CH_colors) +geom_sf(data=wca_shp0_filtered,  lwd=1, fill=NA)
